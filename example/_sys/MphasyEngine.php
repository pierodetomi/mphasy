	...
	class MphasyEngine
	{
		...
		public function ProcessRequest()
		{
			...
			if($template === FALSE || $template == null)
			{
				...
				// does not exist!
				$this->ShowErrorView('The template specified for the view does not exist.');
				return;
			}
			
			$template = $this->ProcessTemplate($template, $errorMsg);
			
			if($template == null)
			{
				$this->ShowErrorView($errorMsg);
				return;
			}
		...
		private function ShowErrorView($errorMessage)
		{
			$this->ViewModel = new ErrorViewModel();
			$this->ViewModel->SetError($errorMessage);
			
			$view = $this->ReadSystemViewFile('Error');
		...
		private function ProcessTemplate($template, &$errorMsg)
		{
			while( TRUE )
			{
				...
				if($argsCount != 3 && $argsCount != 1)
				{
					...
					$errorMsg = 'Your view template contains a malformed <strong>&#64;PartialView()</strong> directive (i.e. wrong number of arguments in call?).';
					return null;
				}
				...
				if($argsCount == 3)
				{
					...
					if( !class_exists($viewModelFullName) )
					{
						...
						$errorMsg = 'Your view template contains a call to <strong>&#64;PartialView()</strong> directive that specifies a view&dash;model that doesn\'t exist.<br />The requested view is <strong>'.$viewName.'</strong> and the view&dash;model (that doesn\'t exist) is <strong>'.$viewModelName.'</strong>';
						return null;
					}
					...
					if( !method_exists($this->{$viewModelFullName}, $viewModelMethod) )
					{
						...
						$errorMsg = 'No public function with name <strong>'.$viewModelMethod.'</strong> exists in <strong>'.$viewModelFullName.'</strong> view&dash;model class for the specified <strong>&#64;PartialView()</strong> directive call in your view template.';
						return null;
					}
				...
				if( $view === FALSE || $view == null )
				{
					...
					$errorMsg = 'Your view template contains a call to <strong>&#64;PartialView()</strong> directive that specifies a view that doesn\'t exist.<br />The requested view (that doesn\'t exist) is <strong>'.$viewName.'</strong>';
					return null;
				}
		...
		private function ReadSystemViewFile($viewName)
        {
            $viewPath = SYS_VIEWS_PATH.$viewName.'.html';
